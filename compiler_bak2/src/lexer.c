...
int yylex() {
    ...
    if (lookahead==EOF) {
        if (!at_start_of_line)
            ...
            indent_level--;
            he = hash_table_lookup("<outdent>");
        } else
            he = hash_table_lookup("<end of file>");
    
    } else if (at_start_of_line && column>indent_stack[indent_level]) {
        indent_level++;
        if (indent_level==MAX_INDENT)
            fatal("too many nested blocks");
        indent_stack[indent_level] = column;
        he = hash_table_lookup("<indent>");

    } else if (at_start_of_line && column<indent_stack[indent_level]) {
        if (indent_level==0)
            fatal("unindent does not match any outer indentation level");
        indent_level--;
        if (column>indent_stack[indent_level]) {
    ...
    yylval = he->key;
    line_continues = (he->kind>=TOKEN_PLUS && he->kind<=TOKEN_OPENCL) || (he->kind==TOKEN_EOL);
    at_start_of_line = (he->kind==TOKEN_EOL || he->kind==TOKEN_OUTDENT);
    return he->kind;
}
